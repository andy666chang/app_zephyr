...
#define DT_DRV_COMPAT nuvoton_m031_scc

#include <zephyr/drivers/clock_control.h>
// #include <zephyr/drivers/clock_control/clock_control_m031.h>
#include <zephyr/logging/log.h>
#include <NuMicro.h>
`
LOG_MODULE_REGISTER(clock_control_m031_scc, CONFIG_CLOCK_CONTROL_LOG_LEVEL);

struct m031_scc_config {
	uint32_t clk_base;
	int hxt;
	int lxt;
	int hirc48;
...
static inline int m031_scc_on(const struct device *dev, clock_control_subsys_t subsys)
{
	ARG_UNUSED(dev);

	struct m031_scc_subsys *scc_subsys = (struct m031_scc_subsys *)subsys;
...
static inline int m031_scc_off(const struct device *dev, clock_control_subsys_t subsys)
{
	ARG_UNUSED(dev);
...
static inline int m031_scc_get_rate(const struct device *dev, clock_control_subsys_t subsys,
				       uint32_t *rate)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(subsys);
...
static inline int m031_scc_set_rate(const struct device *dev, clock_control_subsys_t subsys,
...
static inline int m031_scc_configure(const struct device *dev, clock_control_subsys_t subsys,
					void *data)
{
	ARG_UNUSED(dev);
	ARG_UNUSED(data);
...
static struct clock_control_driver_api m031_scc_api = {
	.on = m031_scc_on,
	.off = m031_scc_off,
	.get_rate = m031_scc_get_rate,
	.set_rate = m031_scc_set_rate,
	.configure = m031_scc_configure,
};
...
static int m031_scc_init(const struct device *dev)
{
	const struct m031_scc_config *cfg = dev->config;
...
#define NUMICRO_SCC_INIT(inst)                                                                     \
	static const struct m031_scc_config m031_scc_config_##inst = {                       \
		...
		.core_clock = DT_INST_PROP_OR(inst, core_clock, 0),                                \
                                                                                                   ...
                                                                                                   \
	DEVICE_DT_INST_DEFINE(inst, &m031_scc_init, NULL, NULL, &m031_scc_config_##inst,     \
			      PRE_KERNEL_1, CONFIG_CLOCK_CONTROL_INIT_PRIORITY, &m031_scc_api);
